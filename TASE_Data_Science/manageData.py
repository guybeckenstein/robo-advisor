...
def plotPortfolioComponent(indexes,weights,name):#TODO-FIX HEBREW TEXT
    ...
    plt.pie(weights, labels=stocksNames.values(), autopct='%1.1f%%', shadow=True, startangle=140)
    plt.axis('equal')
    plt.show()
...
def markovich(Num_porSimulation,IsraliIndexes,UsaIndexes, record_percentage_to_predict,name):
    frame = {}
    # map symbol ticker to name
    stocksNames = convertIsraeliIndexToName(IsraliIndexes)
    UsaIndexesNames = convertUsaIndexToName(UsaIndexes)
    stocksNames=list(stocksNames)+UsaIndexes
    ...
    # USA STOCKS
    yf.pdr_override()
    start_date, end_date = getfromAndToDate(10)
    ...
    num_assets = len(IsraliIndexes)+len(UsaIndexes)
    ...
    column_order = ['Returns', 'Volatility', 'Sharpe Ratio'] + [stock + ' Weight' for stock in stocksNames]
    ...
    df.plot.scatter(x='Volatility', y='Returns', c='Sharpe Ratio',
                    cmap='RdYlGn', edgecolors='black', figsize=figSize, grid=True)
    ...
    multseries = pd.Series([1, 1, 1] + [100 for stock in stocksNames],
                           index=['Returns', 'Volatility', 'Sharpe Ratio'] + [stock + ' Weight' for stock in stocksNames])
    with pd.option_context('display.float_format', '%{:,.2f}'.format):
        plt.figtext(0.2, 0.15, "Max returns Porfolio: \n" + df.loc[red_num[0]].multiply(multseries).to_string(),
                    bbox=dict(facecolor='red', alpha=0.5), fontsize=11, style='oblique', ha='center', va='center',fontname='Arial',
                    wrap=True)
        plt.figtext(0.45, 0.15, "Safest Portfolio: \n" + df.loc[yellow_num[0]].multiply(multseries).to_string(),
                    bbox=dict(facecolor='yellow', alpha=0.5), fontsize=11, style='oblique', ha='center', va='center',fontname='Arial',
                    wrap=True)
        plt.figtext(0.7, 0.15, "Sharpe  Portfolio: \n" + df.loc[green_num[0]].multiply(multseries).to_string(),
                    bbox=dict(facecolor='green', alpha=0.5), fontsize=11, style='oblique', ha='center', va='center',fontname='Arial',
                    wrap=True)
    #plt.show()
...
def price_forecast(df, record_percentage_to_predict,isIsrael):
    ...
    if(isIsrael==1):
        ...
        df['PCT_change'] = (df['closingIndexPrice'] - df['indexOpeningPrice']) / df['indexOpeningPrice'] * 100.0
...
def setPortfolioData(command,sybmolIndex):
    ...
    if(command=="get_past_10_years_history"):
        for i in range(len(sybmolIndex)):
            appUrl=getIndexHistory(setting.indexEndOfDayHistoryTenYearsUpToday,sybmolIndex[i],10)
            portfolio[i] = getSymbolInfo(appUrl,"pastTenYearsHistory"+str(sybmolIndex[i]))
...
def getIndexHistory(appName, indexId,numOfYears):
    ...
    endmonth = today.month
    endday = today.day
    return getAppUrlWithDateAndIndex(appName, startyear, startmonth, startday, endyear, endmonth, endday, indexId)

def getAppUrlWithDate(appName, year, month, day):  # /tase/prod/api/v1/index_end_of_day_data/2022/11/22
    return getAppUrlWithoutDate(appName) + "/" + str(year) + "/" + str(month) + "/" + str(day)

def getAppUrlWithDateAndIndex(appName,startYear,startMounth,startDay,endYear,endMonth,endDay,indexName):  
    return getAppUrlWithoutDate(appName) +str(indexName)+ "&fromDate="+str(startYear)+"-"+str(startMounth)+"-"+str(startDay)+"&toDate="+str(endYear)+"-"+str(endMonth)+"-"+str(endDay)

def getAppUrlWithDate(appName, year, month, day,indexId):  # /tase/prod/api/v1/index_end_of_day_data/2022/11/22?indexId=142
    return getAppUrlWithoutDate(appName) + "/" + str(year) + "/" + str(month) + "/" + str(day)+"?indexId="+str(indexId)
...
def createsJsonFile(json_obj, nameProduct):
    ...
    with open(last_element + ".json", "w") as f:# Use the `dump()` function to write the JSON data to the file
...
def convertIsraeliIndexToName(IsraliIndexes):
    ...
    for i, index in enumerate(IsraliIndexes):
        ...
        hebrew_parts = re.findall(hebrew_pattern, text)  # find all the Hebrew parts in the text
        for hebrew_part in hebrew_parts:
            hebrew_part_reversed = ''.join(
                reversed(hebrew_part))  # reverse the order of the Hebrew characters in the part
            text = text.replace(hebrew_part,
                                hebrew_part_reversed)  # replace the original part with the reversed part in the text
        stocksNames[i]=text
