...
def plotMarkowitzGraph(sectorsList, threeBestSectorsWeights, min_variance_port, sharpe_portfolio,
...
def plotGiniGraph(sectorsList, threeBestSectorsWeights, min_variance_port,
                  sharpe_portfolio,
                  max_profolios_annual, max_ginis, df)->plt:
    ...
    with pd.option_context("display.float_format", "%{:,.2f}".format):
        plt.figtext(
            ...
            + "Annual max loss: " + str(round(max_profolios_annual.iloc[0][0]
                                              - 1.65*max_profolios_annual.iloc[0][1], 2))
            + "%\n"
...
def plotbb_strategy_stock(stockprices, buy_price, sell_price)->plt:
...
def plotbb_strategy_Portfolio(stockprices, buy_price, sell_price, newPortfolio)->plt:
    stockprices[['Adj Close', 'Lower', 'Upper']].plot(figsize=(10, 4))
    plt.scatter(stockprices.index, buy_price, marker='^', color='green', label='BUY', s=200)
    plt.scatter(stockprices.index, sell_price, marker='v', color='red', label='SELL', s=200)
...
def plotThreePortfoliosGraph(min_variance_port, sharpe_portfolio, max_returns, threeBestSectorsWeights,
                             sectorsList, pctChangeTable)->plt:
    # plot frontier, max sharpe & min Volatility values with a scatterplot
    figSize_X = 10
    figSize_Y = 8
    figSize = (figSize_X, figSize_Y)
    ...
    pctChangeTable['yield_1_percent'] = (pctChangeTable['yield_1'] - 1) * 100
    pctChangeTable['yield_2_percent'] = (pctChangeTable['yield_2'] - 1) * 100
    pctChangeTable['yield_3_percent'] = (pctChangeTable['yield_3'] - 1) * 100

    pctChangeTable['yield_1_percent'].plot(figsize=figSize, grid=True, color="yellow", linewidth=2, label="saftest",
                                   legend=True, linestyle="dashed")
    pctChangeTable['yield_2_percent'].plot(figsize=figSize, grid=True, color="green", linewidth=2, label="sharpe",
                                   legend=True, linestyle="dashed")
    pctChangeTable['yield_3_percent'].plot(figsize=figSize, grid=True, color="red", linewidth=2, label="max return",
                                   legend=True, linestyle="dashed")
...
def plotpriceForcast(stockSymbol, df, isDataGotFromTase)->plt:
...
def plotDistributionOfStocks(stockNames, pctChangeTable) -> plt:
    plt.subplots(figsize=(8, 8))
    plt.legend()
    plt.xlabel('Return', fontsize=12)
    plt.ylabel('Distribution', fontsize=12)
    for i in range(len(stockNames)):
        sns.distplot(pctChangeTable[stockNames[i]][::30] * 100, kde=True, hist=False, rug=False,
                     label=stockNames[i])
    plt.grid(True)
    plt.legend()
    return plt
...
def plotDistributionOfPortfolio(yieldsList)->plt:
    ...
    for i in range(len(yieldsList)):
        ...
        sns.distplot(pd.Series(monthlyChanges[i]), kde=True, hist_kws={'alpha': 0.2}, norm_hist=False,
