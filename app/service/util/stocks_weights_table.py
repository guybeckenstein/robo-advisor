...
def draw_all_and_save_as_png(file_name: str, symbols: list[int], weights: list[float], descriptions: list[str]) -> None:
    """
    Stocks Representation
    """
    if len(weights) != len(symbols) or len(weights) != len(descriptions):
        raise ValueError("Input lists must have the same length.")
    image, draw = create_blank_image(num_rows=len(symbols))
    draw_table_header(draw=draw)
    draw_table_rows(num_rows=len(weights), symbols=symbols, weights=weights, descriptions=descriptions, draw=draw)
...
def draw_table_header(draw: ImageDraw) -> None:
...
def draw_table_rows(num_rows: int, symbols: list[str], weights: list[float], descriptions: list[str],
                    draw: ImageDraw) -> None:
    x0: list[int, int, int] = [
    ...
    for row_idx, (symbol, weight, description) in enumerate(zip(symbols, weights, descriptions)):
